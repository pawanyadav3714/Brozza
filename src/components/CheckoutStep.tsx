...
export default function CheckoutStep({ onBack, onProceed }: CheckoutStepProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="mb-12">
        <h2 className="text-5xl font-black text-white mb-4 tracking-tight">Delivery Details</h2>
        <p className="text-gray-400 text-xl">Confirm where your gourmet meal should be delivered.</p>
      </div>
            ...
            {history.map((addr) => {
              ...
              return (
                <motion.button
                  ...
                  className={`text-left p-6 rounded-[2rem] border transition-all shadow-2xl backdrop-blur-xl cursor-pointer ${
                    isCurrent 
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isCurrent && (
                      <span className="text-[10px] font-black uppercase tracking-wider bg-red-500 text-white px-2.5 py-1 rounded-full">
                        Selected
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-start gap-3 text-sm text-gray-300 line-clamp-2 bg-black/20 p-3 rounded-xl border border-white/5">
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white/5 backdrop-blur-2xl p-10 rounded-[3rem] border border-white/10 shadow-3xl">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between ml-1 flex-wrap gap-2">
            ...
            {user ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 backdrop-blur-md">
                <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
                <span>Auto-captured from account</span>
              </span>
            ) : (
              <button
                type="button"
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-white/10 hover:bg-white/15 border border-white/10 text-gray-300 hover:text-white transition-all cursor-pointer disabled:opacity-50"
              >
                <LogIn className="w-3.5 h-3.5 text-red-400" />
                <span>{isSigningIn ? 'Connecting...' : 'Sign in to auto-capture'}</span>
              </button>
            )}
          </div>
          <div className="relative group">
            <User className={`absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 transition-colors ${
            ...
            <input
              ...
              className={`w-full pl-16 pr-12 py-5 bg-black/40 border rounded-2xl text-white placeholder:text-gray-600 focus:bg-black/60 focus:ring-2 focus:ring-red-500/50 transition-all outline-hidden font-bold ${
                formData.name ? 'border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.08)]' : 'border-white/5'
              }`}
            />
            {formData.name && (
        ...
        <div className="space-y-3">
          ...
          <div className="relative group">
            <Phone className={`absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 transition-colors ${
            ...
            <input
              ...
              className={`w-full pl-16 pr-12 py-5 bg-black/40 border rounded-2xl text-white placeholder:text-gray-600 focus:bg-black/60 focus:ring-2 focus:ring-red-500/50 transition-all outline-hidden font-bold ${
                formData.phone ? 'border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.08)]' : 'border-white/5'
              }`}
            />
            {formData.phone && (
              <div className="absolute right-5 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between ml-1 flex-wrap gap-2">
            <label className="text-sm font-black text-gray-400 uppercase tracking-widest">Delivery Address</label>
          ...
          {recentAddresses.length > 0 && (
            <div className="bg-black/30 border border-white/10 rounded-2xl p-3.5 space-y-2.5">
              <div className="flex items-center justify-between text-[11px] font-bold text-gray-400 px-1">
                <span className="flex items-center gap-1.5 uppercase tracking-wider text-gray-300 font-black">
                  <History className="w-3.5 h-3.5 text-red-500" />
                  Recent Order Addresses
                </span>
                <span className="text-gray-500 font-medium">Tap to auto-type</span>
              </div>
                ...
                {recentAddresses.map((recent, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 max-w-[280px] text-left p-3.5 rounded-xl border text-xs transition-all cursor-pointer ${
                        isSelected
                          ? 'bg-red-500/20 border-red-500/50 text-white shadow-lg shadow-red-900/30'
                    ...
                    >
                      ...
                      <p className="text-[11px] text-gray-400 line-clamp-1 font-medium leading-relaxed">{recent.address}</p>
                      {recent.phone && (
                        <p className="text-[10px] text-gray-500 mt-1.5 font-mono">Ph: {recent.phone}</p>
                      )}
                    </button>
                  );
                })}
          ...
          <div className="relative group">
            <MapPin className={`absolute left-5 top-6 w-6 h-6 transition-colors ${
            ...
            <textarea
              ...
              className={`w-full pl-16 pr-12 py-5 bg-black/40 border rounded-2xl text-white placeholder:text-gray-600 focus:bg-black/60 focus:ring-2 focus:ring-red-500/50 transition-all outline-hidden resize-none font-bold ${
                formData.address ? 'border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.08)]' : 'border-white/5'
              }`}
            />
            {formData.address && (
              <div className="absolute right-5 top-6 flex items-center gap-1.5 pointer-events-none">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
              </div>
            )}
          </div>

          {/* Real-time matching suggestions while typing */}
          {matchingSuggestions.length > 0 && (
            <motion.div 
              initial={{ opacity: 0, y: -6 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-neutral-900/95 border border-white/10 rounded-2xl p-2.5 shadow-2xl backdrop-blur-xl"
            >
              <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 px-3 py-1.5 flex items-center gap-1.5">
                <Sparkles className="w-3 h-3 text-red-500" />
                Matching Detailed Addresses from Recent Orders
              </div>
              <div className="space-y-1 mt-1">
                {matchingSuggestions.map(s => (
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2.5 rounded-xl hover:bg-white/10 flex items-center justify-between gap-3 text-xs transition-colors cursor-pointer group"
                  >
                    <div className="flex items-start gap-2.5 min-w-0">
                      <MapPin className="w-4 h-4 text-red-400 mt-0.5 shrink-0 group-hover:scale-110 transition-transform" />
                      <div className="min-w-0">
                        <span className="font-bold text-white block truncate">{s.address}</span>
                        {s.phone && <span className="text-[10px] text-gray-400 block font-medium">Contact: {s.phone}</span>}
                      </div>
                    </div>
                    <span className="text-[10px] font-bold text-red-400 shrink-0 uppercase tracking-wider flex items-center gap-1 group-hover:underline">
                      Auto-fill
        ...
        <button
          type="submit"
          className="w-full bg-red-600 text-white py-5 rounded-2xl font-black text-xl hover:bg-red-700 transition-all shadow-xl shadow-red-900/40 active:scale-95"
        >
